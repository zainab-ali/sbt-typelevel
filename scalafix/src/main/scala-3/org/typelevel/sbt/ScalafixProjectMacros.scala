...
private[sbt] object ScalafixProjectMacros {
  ...
  private def errorMsg: String =
    "tlScalafixProject must be directly assigned to a val, such as `val x = tlScalafixProject`. Alternatively, you can use `org.typelevel.sbt.ScalafixProject.apply`"
}
